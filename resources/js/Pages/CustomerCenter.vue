        ...
        <div class="p-4 bg-zinc-950 min-h-[calc(100vh-64px)] flex flex-col">
            ...
            <div class="max-w-[1600px] w-full mx-auto mb-6 flex justify-between items-end">
                <div class="flex flex-col">
                    ...
                    <p class="text-zinc-500 text-sm max-w-2xl leading-relaxed">Manage clients, track transaction history, and monitor accounts receivable exposure in real-time.</p>
                </div>
                ...
                <div class="flex gap-4 items-center">
                    ...
                    <Menu ref="menu" :model="actionOptions" :popup="true" class="!bg-zinc-900 !border-zinc-800 !p-2 !rounded-xl !min-w-[200px]" :pt="{
            ...
            <div class="max-w-[1600px] w-full mx-auto grid grid-cols-12 gap-4 items-start flex-1 min-h-0">
                            ...
                            <template #option="{ option }">
                                <div class="flex flex-col gap-2 w-full">
                                    <div class="flex justify-between items-center w-full">
                                        <span class="text-[9px] font-bold font-mono tracking-widest uppercase" :class="selectedCustomer?.id === option.id ? 'text-sky-400' : 'text-zinc-600'">{{ option.customer_code }}</span>
                ...
                <main v-if="selectedCustomer" class="col-span-12 lg:col-span-9 flex flex-col gap-4 min-h-0">
                            ...
                            <div class="relative z-10 flex flex-col">
                                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-6 pb-6 border-b border-zinc-800/60">
                                    <div class="flex flex-col flex-1">
                                        <div class="flex items-center gap-4 mb-3">
                                            ...
                                            <div class="flex flex-wrap gap-2">
                                                <span class="text-[9px] font-bold px-3 py-1 bg-zinc-800/80 border border-zinc-700 rounded-full text-zinc-400 uppercase tracking-widest font-mono">CUSTOMER</span>
                                        ...
                                        <div class="flex flex-wrap gap-6 text-zinc-500 text-xs font-mono">
                                    ...
                                    <div v-if="can('manage-customers')" class="flex gap-3">
                                        <button @click="editCustomer" class="w-12 h-12 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-800 transition-all flex items-center justify-center">
                                            <i class="pi pi-pencil"></i>
                                        </button>
                                        <button @click="deleteCustomer" class="w-12 h-12 rounded-xl bg-zinc-900 border border-zinc-800 text-rose-500 hover:text-rose-400 hover:bg-rose-500/5 transition-all flex items-center justify-center">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                    <div class="p-4 bg-zinc-950/80 border border-zinc-800 rounded-2xl flex flex-col shadow-lg ring-1 ring-white/5">
                                        <span class="text-[9px] font-bold text-zinc-500 uppercase tracking-[0.2em] mb-2 font-mono">Credit Limit</span>
                                        <span class="text-xl font-bold text-white font-mono">{{ formatCurrency(selectedCustomer.credit_limit || 0) }}</span>
                                    </div>
                                    <div class="p-4 bg-zinc-950/80 border border-zinc-800 rounded-2xl flex flex-col shadow-lg ring-1 ring-white/5"
                                         :class="creditUtilization >= 90 ? 'ring-rose-500/20' : 'ring-white/5'">
                                        <span class="text-[9px] font-bold text-zinc-500 uppercase tracking-[0.2em] mb-2 font-mono">Outstanding Balance</span>
                                        <span class="text-xl font-bold font-mono" :class="Number(selectedCustomer.exposure) > 0 ? 'text-amber-400' : 'text-zinc-500'">{{ formatCurrency(selectedCustomer.exposure || 0) }}</span>
                                    </div>
                                    <div class="p-4 bg-zinc-950/80 border border-zinc-800 rounded-2xl flex flex-col justify-between shadow-lg ring-1 ring-white/5 lg:col-span-2">
                                        <div class="flex justify-between items-center mb-2">
                                            <span class="text-[9px] font-bold text-zinc-500 uppercase tracking-[0.2em] font-mono">Credit Exposure</span>
                                            <span class="text-[10px] font-bold font-mono" :class="creditUtilization >= 90 ? 'text-rose-400' : 'text-sky-400'">{{ creditUtilization }}%</span>
                                        </div>
                                        <div class="h-2 bg-zinc-900 rounded-full overflow-hidden mb-1">
                                        ...
                                        <div class="flex justify-between items-center">
                                            <span class="text-[8px] text-zinc-600 font-bold uppercase tracking-widest">Available Credit</span>
                        ...
                        <section class="bg-zinc-900/40 border border-zinc-800/80 rounded-2xl overflow-hidden shadow-2xl flex flex-col backdrop-blur-sm min-h-[500px]">
                            <!-- Tab Bar -->
                            <div class="px-8 border-b border-zinc-800/60 bg-zinc-900/60 flex items-center gap-8 h-16 divide-x divide-zinc-800/40">
                                <button @click="activeTab = 'orders'"
                                    ...
                                    :class="activeTab === 'orders' ? 'text-sky-400' : 'text-zinc-500 hover:text-zinc-300'">
                                    Recent Orders
                                ...
                                <button @click="activeTab = 'invoices'"
                                    ...
                                    :class="activeTab === 'invoices' ? 'text-emerald-400' : 'text-zinc-500 hover:text-zinc-300'">
                                    Invoices
                                ...
                                <button @click="activeTab = 'payments'"
                                    class="h-full px-8 text-[10px] font-bold uppercase tracking-[0.2em] transition-all relative border-none bg-transparent cursor-pointer"
                                    :class="activeTab === 'payments' ? 'text-indigo-400' : 'text-zinc-500 hover:text-zinc-300'">
                                    Payments
                                ...
                                <button @click="activeTab = 'statement'"
                                    class="h-full px-8 text-[10px] font-bold uppercase tracking-[0.2em] transition-all relative border-none bg-transparent cursor-pointer"
                                    :class="activeTab === 'statement' ? 'text-amber-400' : 'text-zinc-500 hover:text-zinc-300'">
                                    Full Statement
                                ...
                                <div class="ml-auto pl-4">
                                    <span class="bg-zinc-950 text-zinc-600 px-3 py-1 rounded text-[10px] font-bold border border-zinc-800/50 font-mono tracking-tighter uppercase whitespace-nowrap">
                        ...
                        <div v-if="activeTab === 'orders'" class="flex-1 min-h-0">
                            ...
                            >
                                ...
                                <Column field="order_date" header="Date" style="width: 120px">
                                    <template #body="{ data }">
                                        <span class="font-mono text-[11px] text-zinc-500">{{ data.order_date }}</span>
                                    </template>
                                </Column>
                                ...
                                <Column field="total_amount" header="Order Value" style="width: 160px">
                                    <template #body="{ data }">
                                        <span class="text-white font-bold font-mono text-sm">{{ formatCurrency(data.total_amount) }}</span>
                                    </template>
                                </Column>
                                ...
                                <Column header="Notes">
                                    <template #body="{ data }">
                                        <span class="text-zinc-600 text-xs italic truncate block max-w-[200px]">{{ data.notes || '—' }}</span>
                                    </template>
                                </Column>
                            </DataTable>
                        </div>
                        ...
                        <div v-if="activeTab === 'invoices'" class="flex-1 min-h-0">
                            ...
                            >
                                ...
                                <Column field="invoice_date" header="Date" style="width: 120px">
                                    <template #body="{ data }">
                                        <span class="font-mono text-[11px] text-zinc-500">{{ data.invoice_date }}</span>
                                    </template>
                                </Column>
                                ...
                                <Column field="balance_due" header="Balance Due" style="width: 160px">
                                    <template #body="{ data }">
                                        <span class="font-bold font-mono text-sm" :class="Number(data.balance_due) > 0 ? 'text-rose-400' : 'text-zinc-600'">
                        ...
                        <div v-if="activeTab === 'payments'" class="flex-1 min-h-0">
                            ...
                            >
                                ...
                                <Column field="payment_date" header="Date" style="width: 120px">
                                    <template #body="{ data }">
                                        <span class="font-mono text-[11px] text-zinc-500">{{ data.payment_date }}</span>
                                    </template>
                                </Column>
                                ...
                                <Column field="amount" header="Total Amount" style="width: 160px">
                                    <template #body="{ data }">
                                        <span class="text-white font-bold font-mono text-sm">{{ formatCurrency(data.amount) }}</span>
                                    </template>
                                </Column>
                                <Column field="unallocated_amount" header="Unallocated" style="width: 160px">
                                    <template #body="{ data }">
                                        <span class="font-bold font-mono text-sm" :class="Number(data.unallocated_amount) > 0 ? 'text-amber-400' : 'text-zinc-600'">
                                ...
                                <Column header="Ref / Method">
                                    <template #body="{ data }">
                                        <span class="text-zinc-500 text-xs font-mono">{{ data.method }} / {{ data.reference || '—' }}</span>
                                    </template>
                                </Column>
                            </DataTable>
                        </div>
                        ...
                        <div v-if="activeTab === 'statement'" class="flex-1 min-h-0">
                            ...
                            >
                                ...
                                <Column field="date" header="Date" style="width: 110px">
                                    <template #body="{ data }">
                                        <span class="font-mono text-[11px] text-zinc-500">{{ data.date }}</span>
                                    </template>
                                </Column>
                                ...
                                <Column field="reference" header="Reference" style="width: 150px">
                                    <template #body="{ data }">
                                        ...
                                        <span v-else class="font-mono text-xs text-zinc-500">{{ data.reference }}</span>
                                    </template>
                                </Column>
                                ...
                                <Column field="running_balance" header="Running Balance" style="width: 150px; text-align: right">
                                    <template #body="{ data }">
                                        <span class="font-bold font-mono text-sm" :class="Number(data.running_balance) > 0 ? 'text-white' : 'text-zinc-500'">
                    ...
                    <main v-else class="col-span-12 lg:col-span-9 flex flex-col items-center justify-center bg-zinc-900/40 border border-zinc-800/80 rounded-2xl p-32 backdrop-blur-sm shadow-2xl">
                        <div class="relative mb-12 group">
                            ...
                            <div class="w-32 h-32 rounded-[2rem] bg-zinc-950 border border-zinc-800 flex items-center justify-center relative z-10 shadow-2xl transition-transform duration-500 group-hover:scale-110">
                                <i class="pi pi-users text-5xl text-sky-500/40"></i>
                            </div>
                        </div>
                        <div class="text-center relative z-10">
                            <h2 class="text-2xl font-bold text-white tracking-tight mb-3">No Customer Selected</h2>
                            <p class="text-zinc-500 max-w-sm mx-auto leading-relaxed text-sm">Select a client from the registry on the left to view their detailed transaction history and financial exposure.</p>
                            <div class="mt-8 flex items-center justify-center gap-4 text-[9px] font-bold text-zinc-600 font-mono tracking-widest uppercase">
            ...
            <Dialog
                ...
                :showHeader="false"
            >
                <div class="bg-zinc-950 border border-zinc-800 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden ring-1 ring-white/5">
                    <!-- Dialog Header -->
                    <div class="px-8 py-6 border-b border-zinc-900 bg-zinc-900/50 flex justify-between items-center relative overflow-hidden">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <h2 class="text-white text-xl font-bold tracking-tight m-0">{{ customerForm.id ? 'Modify Record' : 'Create New Client' }}</h2>
                            </div>
                        </div>
                        <button @click="dialogVisible = false" class="w-10 h-10 rounded-xl text-zinc-500 hover:text-white hover:bg-zinc-800 transition-all flex items-center justify-center">
                    ...
                    <div class="p-8 space-y-5">
                        <div class="grid grid-cols-12 gap-4">
                            ...
                            <div class="col-span-4 flex flex-col gap-2">
                                <label class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest font-mono">Registry ID *</label>
                                <InputText v-model="customerForm.customer_code" placeholder="CST-000"
                                           class="!bg-zinc-900/50 !border-zinc-800 !text-sky-400 !h-12 !font-mono focus:!border-sky-500/30"
                                           :class="{'!border-rose-500/50': submitted && !customerForm.customer_code}" />
                            </div>

                            <div class="col-span-6 flex flex-col gap-2">
                                <label class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest font-mono">Email</label>
                                <InputText v-model="customerForm.email" placeholder="client@company.com" class="!bg-zinc-900/50 !border-zinc-800 !text-zinc-300 !h-12 focus:!border-sky-500/30 font-mono text-xs" />
                            </div>
                            <div class="col-span-6 flex flex-col gap-2">
                                <label class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest font-mono">Phone</label>
                                <InputText v-model="customerForm.phone" placeholder="+63 000 000 0000" class="!bg-zinc-900/50 !border-zinc-800 !text-zinc-300 !h-12 focus:!border-sky-500/30 font-mono text-xs" />
                            </div>

                            <div class="col-span-6 flex flex-col gap-2">
                                <label class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest font-mono">Tax / VAT Number</label>
                                <InputText v-model="customerForm.tax_number" placeholder="VAT-000000" class="!bg-zinc-900/50 !border-zinc-800 !text-zinc-300 !h-12 focus:!border-sky-500/30 font-mono text-xs" />
                            </div>
                            <div class="col-span-6 flex flex-col gap-2">
                                <label class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest font-mono">Credit Limit</label>
                                <InputNumber v-model="customerForm.credit_limit" mode="currency" currency="PHP" locale="en-PH"
                                             :pt="{ input: { class: '!bg-zinc-900/50 !border-zinc-800 !text-white !h-12 !font-mono !text-sm' } }" />
                            </div>

                            <div class="col-span-12 flex flex-col gap-2">
                                <label class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest font-mono">Billing Address</label>
                                <Textarea v-model="customerForm.billing_address" rows="2" placeholder="Full billing address..." class="!bg-zinc-900/50 !border-zinc-800 !text-zinc-300 focus:!border-sky-500/30 text-xs" />
                            </div>

                            <div class="col-span-12 flex flex-col gap-2">
                                <label class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest font-mono">Shipping Address</label>
                                <Textarea v-model="customerForm.shipping_address" rows="2" placeholder="Primary delivery address..." class="!bg-zinc-900/50 !border-zinc-800 !text-zinc-300 focus:!border-sky-500/30 text-xs" />
                            </div>

                            <div class="col-span-12 p-4 bg-zinc-900/40 border border-zinc-800/60 rounded-xl flex items-center justify-between">
                                <div>
                                    <span class="text-white font-bold text-sm tracking-tight m-0">Account Visibility</span>
                                    <p class="text-zinc-500 text-[10px] font-mono m-0 mt-0.5 uppercase tracking-tighter">{{ customerForm.is_active ? 'ENABLED — Full System Access' : 'DISABLED — Read-Only Archive' }}</p>
                                </div>
                    ...
                    <!-- Dialog Footer -->
                    <div class="px-8 py-5 border-t border-zinc-900 bg-zinc-900/50 flex justify-end gap-3">
                        <button @click="dialogVisible = false" class="px-5 py-2.5 rounded-xl bg-transparent border border-zinc-800 text-zinc-500 hover:text-white hover:border-zinc-600 font-bold text-[10px] uppercase tracking-widest transition-colors">
                            Cancel
                        </button>
