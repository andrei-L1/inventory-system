        ...
        <div class="p-8 bg-deep min-h-[calc(100vh-64px)] overflow-hidden flex flex-col">
            <div class="max-w-[1600px] w-full mx-auto mb-10 flex justify-between items-end">
                <div class="flex flex-col">
                    ...
                    <h1 class="text-3xl font-bold text-primary tracking-tight m-0 mb-2">Issue Items</h1>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <button @click="submitForm" :disabled="isSubmitting" class="!bg-rose-500 !border-none !text-primary !px-8 !h-12 !font-bold !text-[11px] uppercase tracking-widest shadow-lg shadow-rose-500/10 hover:!bg-rose-400 active:scale-95 transition-all rounded-xl disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <div class="max-w-[1600px] w-full mx-auto flex-1 flex flex-col min-h-0">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                    ...
                    <div class="bg-panel/30 border border-panel-border/50 rounded-2xl p-6 border-l-4 border-l-rose-500 shadow-xl backdrop-blur-sm">
                ...
                <div class="grid grid-cols-12 gap-8 flex-1 min-h-0">
                    <aside class="col-span-12 lg:col-span-3 flex flex-col min-h-0 bg-panel/40 border border-panel-border/80 rounded-2xl overflow-hidden shadow-2xl backdrop-blur-sm">
                        ...
                        <div class="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-8">
                             ...
                             <div class="flex flex-col gap-3">
                                 <label class="text-[10px] font-bold text-muted uppercase tracking-widest font-mono">Reference / Invoice # <span class="text-muted normal-case font-sans tracking-normal">(Optional)</span></label>
                                 <InputText 
                             ...
                             <div class="flex flex-col gap-3">
                                 ...
                                 <textarea v-model="form.notes" placeholder="Optional delivery notes..." class="bg-deep border border-panel-border rounded-xl p-4 text-xs text-secondary h-32 resize-none outline-none focus:border-rose-500/30 transition-all"></textarea>
                             </div>
                        </div>
                    </aside>
                    ...
                    <main class="col-span-12 lg:col-span-9 flex flex-col min-h-0 bg-panel/40 border border-panel-border/80 rounded-2xl overflow-hidden shadow-2xl backdrop-blur-sm">
                        <div class="p-6 border-b border-panel-border bg-panel/60 flex justify-between items-center sticky top-0 z-20 backdrop-blur-md">
                            ...
                            <button @click="addLine" class="px-6 h-10 rounded-xl bg-rose-500 text-primary text-[10px] font-black uppercase tracking-widest hover:bg-rose-400 transition-all active:scale-95 flex items-center gap-2">
                        ...
                        <div class="flex-1 overflow-y-auto custom-scrollbar p-6">
                            <div class="flex flex-col gap-3">
                                <div v-for="(line, index) in form.lines" :key="index" class="p-4 bg-panel/20 border border-panel-border/40 rounded-2xl flex flex-col gap-4 relative group transition-all hover:border-rose-500/20 hover:bg-panel/40">
                                    <div class="grid grid-cols-12 gap-4 items-center">
                                        ...
                                        <div class="col-span-12 lg:col-span-5 flex flex-col gap-1">
                                            ...
                                            <Select 
                                                ...
                                                class="!w-full !bg-deep/80 !border-panel-border !h-10 !rounded-xl !text-xs !flex !items-center focus-within:!border-rose-500/30"
                                            />
                                        </div>
                                        ...
                                        <div class="col-span-6 lg:col-span-2 flex flex-col gap-1">
                                            ...
                                            <Select 
                                                ...
                                                placeholder="Unit" 
                                                dataKey="id"
                                                class="!w-full !bg-deep/80 !border-panel-border !h-10 !rounded-xl !text-[11px] !font-black !flex !items-center focus-within:!border-rose-500/30"
                                            >
                                        ...
                                        <div class="col-span-6 lg:col-span-4 flex flex-col gap-1">
                                            <span class="text-[9px] font-black text-muted tracking-widest uppercase font-mono pl-1">Quantity to Issue</span>
                                            <div class="flex items-center bg-deep/80 border border-panel-border rounded-xl focus-within:border-rose-500/50 transition-all overflow-hidden h-10 group/input"
                                    ...
                                    <div v-if="line.product" class="grid grid-cols-1 container border-t border-panel-border/40 pt-3 mt-1">
                                        <div class="flex items-center gap-6">
                                            ...
                                            <div 
                                                ...
                                                class="flex items-center gap-2 px-3 py-1.5 bg-panel border border-panel-border rounded-lg cursor-pointer hover:border-rose-500/50 transition-all group/stat"
                                            >
                                                <div class="flex flex-col text-left">
                                                    <span class="text-[7px] font-bold text-secondary uppercase tracking-widest font-mono group-hover/stat:text-rose-400 line-clamp-1">Local Stock</span>
                                                ...
                                                <div class="flex flex-col text-left">
                                                    <span class="text-[7px] font-bold text-secondary uppercase tracking-widest font-mono group-hover/stat:text-rose-400 line-clamp-1">Global Pool</span>
                                                    <span class="text-[10px] font-black text-secondary font-mono">
                                                        {{ getScaledQty(line, line.product?.total_qoh) }}
                                                    </span>
                                                </div>
                                                <i class="pi pi-chevron-down text-[8px] text-muted group-hover/stat:text-rose-400"></i>
                                            </div>
                                            ...
                                            <!-- Impact Result -->
                                            <div v-if="Number(line.quantity) > 0 && form.from_location" class="flex-1 flex items-center gap-3 px-4 py-2 bg-deep/40 rounded-lg border border-panel-border/50">
                                                <i class="pi pi-sync text-[10px] text-muted"></i>
                                                <div class="flex items-center gap-2">
                                                    <span class="text-[9px] font-black text-muted uppercase tracking-tighter">Remaining After Issue:</span>
                    ...
                    <div v-for="loc in selectedLineForStock.inventories" 
                        ...
                        <div class="flex flex-col">
                            <span class="text-[10px] font-bold uppercase tracking-tight"
                                  :class="[loc.location_id === form.from_location?.id ? 'text-rose-400' : 'text-secondary group-hover:text-primary']">
                                {{ loc.location_name }}
                            </span>
