                    ...
                    <main class="col-span-12 lg:col-span-9 flex flex-col min-h-0 bg-zinc-900/40 border border-zinc-800/80 rounded-2xl overflow-hidden shadow-2xl backdrop-blur-sm">
                        ...
                        <Popover ref="stockOp" class="!bg-zinc-900 !border-zinc-800 !shadow-2xl">
                            <div v-if="selectedLineForStock" class="w-72 p-4 text-white text-left">
                                <div class="text-[9px] font-black text-violet-500 uppercase tracking-widest mb-3 border-b border-zinc-800 pb-2 flex justify-between">
                                ...
                                <div class="space-y-2 max-h-48 overflow-y-auto custom-scrollbar">
                                    <div v-for="inv in selectedLineForStock.inventories" :key="inv.id" class="flex justify-between items-center text-[10px]">
                                        <span class="text-zinc-400 truncate pr-2 uppercase font-bold" :class="{'text-violet-400': inv.location_id === form.from_location?.id}">
                                            {{ inv.location_name }}
                                        </span>
                        ...
                        <div class="flex-1 overflow-y-auto custom-scrollbar">
                            <DataTable :value="form.lines" class="transfer-grid border-none" :pt="{
                                ...
                                <Column field="uom" header="UNIT" class="!py-6 !px-4">
                                 ...
                                 <Column header="STOCK LEVEL" style="width: 140px">
                                     <template #body="{ data }">
                                        <div class="flex flex-col gap-1 items-end">
                                             <div class="flex items-center gap-2">
                                                 <span class="text-xs font-bold font-mono" :class="isInsufficient(data) ? 'text-rose-400' : 'text-zinc-400'">
                                             ...
                                             <span class="text-[8px] font-bold text-zinc-700 uppercase tracking-widest font-mono">
                                                 {{ form.from_location?.name || 'Pick Source' }}
                                             </span>
                                        </div>
                                     </template>
                                 </Column>
                                ...
                                <Column header="SUMMARY" class="!py-6 !px-8 text-right">
                                    <template #body="{ index, data }">
                                        <div class="flex items-center justify-end gap-5 font-mono">
                                            <div class="flex flex-col items-end gap-1">
                                                <span class="text-[8px] font-bold text-zinc-700 uppercase tracking-widest">Moved</span>
                                                <span class="text-xs font-black text-violet-400">
                                                    {{ data.quantity || 0 }}
                                                    <span class="text-[10px] text-violet-500/50 font-medium ml-1">{{ getUomAbbr(data.uom_id) }}</span>
                                                </span>
                                            </div>
                                            <i class="pi pi-arrow-right text-[10px] text-zinc-800" />
                                            <div class="flex flex-col items-end gap-1">
                                                <span class="text-[8px] font-bold text-zinc-700 uppercase tracking-widest">Post-Transfer</span>
                                                <span class="text-xs font-black text-rose-400">
                                                    {{ (parseFloat(getScaledQty(data, getLocalStock(data))) - (parseFloat(data.quantity) || 0)).toFixed(2) }}
                                                </span>
                                            </div>
