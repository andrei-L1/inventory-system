...
<template>
    <AppLayout>
        ...
        <div class="p-8 bg-deep min-h-screen">
            <!-- Header Section -->
            <div class="max-w-[1600px] mx-auto flex flex-col lg:flex-row justify-between items-start lg:items-end mb-10 gap-8">
                <div class="flex flex-col">
                    ...
                    <h1 class="text-3xl font-bold text-primary tracking-tight m-0 mb-2">Product Catalog</h1>
                    <p class="text-secondary text-sm max-w-2xl leading-relaxed">Manage and organize all products in your system catalog. Track classifications, pricing, and inventory parameters across the organization.</p>
                </div>
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 w-full lg:w-auto">
                    <div class="relative flex-1 sm:w-80">
                        ...
                        <InputText 
                            ...
                            class="w-full !pl-11 !pr-10 !bg-panel/50 !border-panel-border !text-primary !h-12 !text-sm focus:!border-sky-500/30 !rounded-lg transition-all"
                        />
                        <i v-if="search" class="pi pi-times absolute right-4 top-1/2 -translate-y-1/2 cursor-pointer text-xs text-muted hover:text-secondary transition-colors" @click="search = ''"></i>
                    </div>
                    <Button 
                        v-if="can('manage-products')" 
                        label="ADD PRODUCT" 
                        icon="pi pi-plus" 
                        class="!bg-sky-500 !text-primary !border-none !h-12 !font-bold !px-6 !rounded-lg hover:!bg-sky-400 active:scale-95 transition-all shadow-lg shadow-sky-500/10"
                        @click="openNew" 
                    />
                </div>
            </div>
            ...
            <div class="max-w-[1600px] mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div class="bg-panel/40 border border-panel-border/60 rounded-xl p-5 flex items-center gap-5 group hover:border-sky-500/20 transition-all duration-500 shadow-sm hover:shadow-sky-500/5">
                    <div class="w-14 h-14 rounded-lg bg-deep border border-panel-border flex items-center justify-center text-sky-400 group-hover:scale-105 transition-transform duration-500">
                        <i class="pi pi-box text-xl"></i>
                    </div>
                    <div class="flex flex-col">
                        <span class="text-2xl font-bold text-primary tracking-tight leading-none">{{ stats.total }}</span>
                        <span class="text-[10px] font-bold text-secondary uppercase tracking-widest mt-2 font-mono">Total Products</span>
                    </div>
                </div>
                <div class="bg-panel/40 border border-panel-border/60 rounded-xl p-5 flex items-center gap-5 group hover:border-emerald-500/20 transition-all duration-500 shadow-sm hover:shadow-emerald-500/5">
                    <div class="w-14 h-14 rounded-lg bg-deep border border-panel-border flex items-center justify-center text-emerald-400 group-hover:scale-105 transition-transform duration-500">
                        <i class="pi pi-check-circle text-xl"></i>
                    </div>
                    <div class="flex flex-col">
                        <span class="text-2xl font-bold text-primary tracking-tight leading-none">{{ stats.active }}</span>
                        <span class="text-[10px] font-bold text-secondary uppercase tracking-widest mt-2 font-mono">Available Items</span>
                    </div>
                </div>
                <div class="bg-panel/40 border border-panel-border/60 rounded-xl p-5 flex items-center gap-5 group hover:border-amber-500/20 transition-all duration-500 shadow-sm hover:shadow-amber-500/5">
                    <div class="w-14 h-14 rounded-lg bg-deep border border-panel-border flex items-center justify-center text-amber-400 group-hover:scale-105 transition-transform duration-500">
                        <i class="pi pi-dollar text-xl"></i>
                    </div>
                    <div class="flex flex-col">
                        <span class="text-2xl font-bold text-primary tracking-tight leading-none">{{ formatCurrency(stats.totalValue) }}</span>
                        <span class="text-[10px] font-bold text-secondary uppercase tracking-widest mt-2 font-mono">Total Catalog Value</span>
                    </div>
                </div>
            </div>

            <!-- Central Registry Table -->
            <div class="max-w-[1600px] mx-auto bg-panel/50 border border-panel-border/80 rounded-xl overflow-hidden shadow-2xl backdrop-blur-sm">
                    ...
                    :pt="{
                        ...
                        paginator: {
                            ...
                            pagelink: ({ props, state, context }) => ({
                                class: context.active ? '!bg-sky-500 !text-primary !rounded-md' : '!text-secondary hover:!text-primary'
                            })
                        }
                    }"
                >
                    ...
                    <Column field="product_code" header="Part Number (MPN)" style="width: 140px">
                        <template #body="{ data }">
                            <span class="font-mono text-[10px] bg-deep text-sky-400 px-3 py-1 rounded border border-sky-500/20 shadow-[0_0_15px_rgba(56,189,248,0.05)]">{{ data.product_code || '---' }}</span>
                        </template>
                    </Column>

                    <Column field="sku" header="SKU" style="width: 160px">
                        <template #body="{ data }">
                            <span class="font-mono text-[11px] text-secondary tracking-widest uppercase">{{ data.sku }}</span>
                        </template>
                    </Column>
                    ...
                    <Column field="selling_price" header="Price" style="width: 130px">
                        <template #body="{ data }">
                            <span class="font-mono font-bold text-primary text-sm">{{ data.formatted_selling_price }}</span>
                        </template>
                    </Column>
                        ...
                        <template #body="{ data }">
                            <div class="flex flex-col items-start gap-1" @click.stop>
                                <div class="flex items-center gap-2 cursor-help group/stock" @click="toggleStock($event, data)">
                                    ...
                                    <i class="pi pi-info-circle text-[10px] text-muted group-hover/stock:text-sky-500/50 transition-colors"></i>
                                </div>
                    ...
                    <Column header="Actions" style="width: 100px" v-if="can('manage-products')">
                        <template #body="{ data }">
                            <div class="flex items-center gap-2" @click.stop>
                                <Button icon="pi pi-pencil" class="!text-secondary hover:!text-sky-400 hover:!bg-sky-500/10 !w-9 !h-9 !p-0 !rounded-lg !border-none transition-all" @click="editProduct(data)" />
                                <Button icon="pi pi-trash" class="!text-secondary hover:!text-red-400 hover:!bg-red-500/10 !w-9 !h-9 !p-0 !rounded-lg !border-none transition-all" @click="deleteProduct(data)" />
                            </div>
                        </template>
                    </Column>
                </DataTable>
            ...
            <Dialog 
                ...
                :showHeader="false"
            >
                <div class="bg-deep border border-panel-border rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.8)] flex flex-col md:flex-row h-[85vh] overflow-hidden animate-in fade-in zoom-in duration-500 ring-1 ring-white/5">
                    
                    <!-- Sidebar Navigation -->
                    <aside class="w-full md:w-64 bg-panel/50 border-r border-panel-border flex flex-col pt-10 px-6 gap-8">
                        <div class="flex flex-col gap-1 px-2">
                            <div class="text-[9px] font-bold text-sky-500 tracking-[0.3em] font-mono leading-none mb-1">RECORD_DETAILS</div>
                            <h3 class="text-primary text-lg font-bold tracking-tighter m-0 whitespace-nowrap">Product Details</h3>
                        </div>
                        ...
                        <div class="mt-auto pb-10 px-2">
                            <div class="text-[9px] font-bold text-muted font-mono tracking-tighter uppercase mb-1">Access Level</div>
                            <div class="text-[10px] text-secondary font-mono flex items-center gap-2">
                    ...
                    <section class="flex-1 flex flex-col min-w-0">
                        ...
                        <div class="flex-1 overflow-y-auto p-10 bg-[radial-gradient(circle_at_top_right,rgba(56,189,248,0.03),transparent_40%)]">
                            ...
                            <div class="flex justify-between items-center mb-10 pb-6 border-b border-panel-border">
                                ...
                                <Button icon="pi pi-times" class="!text-muted hover:!text-primary !bg-transparent !border-none !w-10 !h-10 hover:!bg-panel transition-colors" @click="dialogVisible = false" :disabled="saving" />
                            </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
                                    <div class="flex flex-col gap-3 md:col-span-2">
                                        <div class="flex justify-between items-center">
                                            <label class="text-[10px] font-bold text-secondary uppercase tracking-widest font-mono">Product Name</label>
                                        ...
                                        <InputText v-model="product.name" placeholder="E.g. Wireless Mouse X10" 
                                                   class="!bg-panel/50 !border-panel-border !text-primary !h-14 !text-lg !font-bold focus:!border-sky-500/40 transition-all !px-5"
                                                   :class="{'!border-red-500/50': errors.name}" />
                                    </div>
                                    <div class="flex flex-col gap-3">
                                        <div class="flex justify-between items-center">
                                            <label class="text-[10px] font-bold text-secondary uppercase tracking-widest font-mono">Part Number (MPN)</label>
                                        ...
                                        <InputText v-model="product.product_code" placeholder="Manufacturer's unique identifier..." 
                                                  ...
                                                  class="!bg-panel/50 !border-panel-border !text-sky-400 !h-12 !font-mono !px-4 focus:!border-sky-500/30 disabled:opacity-50 disabled:cursor-not-allowed"
                                    ...
                                    <div class="flex flex-col gap-3">
                                        <div class="flex justify-between items-center">
                                            <label class="text-[10px] font-bold text-secondary uppercase tracking-widest font-mono">Product SKU</label>
                                    ...
                                    <div class="flex flex-col gap-3">
                                        <div class="flex justify-between items-center">
                                            <label class="text-[10px] font-bold text-secondary uppercase tracking-widest font-mono">Category</label>
                                        ...
                                        <Select v-model="product.category_id" :options="categories" optionLabel="name" optionValue="id" 
                                                class="!bg-panel/50 !border-panel-border !text-primary !h-12"
                                                :class="{'!border-red-500/50': errors.category_id}" />
                                    </div>

                                    <div class="flex flex-col gap-3">
                                        <div class="flex justify-between items-center">
                                            <label class="text-[10px] font-bold text-secondary uppercase tracking-widest font-mono">Preferred Vendor (Owner)</label>
                                            <span class="text-muted text-[9px] font-bold uppercase tracking-widest font-mono italic">Optional</span>
                                        </div>
                                    ...
                                    <div class="flex flex-col gap-3 md:col-span-2">
                                        <div class="flex justify-between items-center">
                                            <label class="text-[10px] font-bold text-secondary uppercase tracking-widest font-mono">Unit of Measure</label>
                                        ...
                                        <Select v-model="product.uom_id" :options="uoms" optionLabel="name" optionValue="id" 
                                                :disabled="isEditing && product.has_history"
                                                class="!bg-panel/50 !border-panel-border !text-primary !h-12 disabled:opacity-50"
                                        ...
                                        <div v-if="!product.id && isNonBaseUomSelected" class="mt-2 p-3 bg-[radial-gradient(ellipse_at_top,rgba(56,189,248,0.05),transparent)] border border-sky-500/20 rounded-lg animate-in fade-in slide-in-from-top-2 duration-300">
                                            <div class="flex items-center justify-between mb-3 border-b border-sky-500/10 pb-2">
                                                ...
                                                <span class="text-[8px] font-bold text-secondary font-mono tracking-tighter">1 {{ uoms.find(u => u.id === product.uom_id)?.abbreviation }} =</span>
                                            </div>
                                            <div class="flex items-center gap-4">
                                                <div class="relative flex-1">
                                                    <InputNumber v-model="product.initial_conversion_factor" placeholder="Enter quantity..." 
                                                                inputClass="!bg-deep !border-sky-500/30 !text-primary !h-11 !w-full !px-3 !font-black !font-mono !text-lg focus:!border-sky-500 shadow-inner" />
                                                    <div class="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-secondary uppercase font-mono tracking-tighter">{{ getBaseUomForSelected?.abbreviation ?? '???' }}</div>
                                                </div>
                                                <div class="flex flex-col gap-0.5 max-w-[120px]">
                                                    <span class="text-[7px] text-secondary font-bold uppercase tracking-[0.1em] font-mono leading-none">Atomic Rule</span>
                                        ...
                                        <p v-if="isEditing && product.has_history" class="text-[8px] font-bold text-muted uppercase tracking-widest font-mono mt-1 italic">UOM locked due to audit history</p>
                                    </div>
                                    <div class="flex flex-col gap-3 md:col-span-2">
                            ...
                            <div v-show="activeTab === 'inventory'" class="animate-in slide-in-from-right-4 duration-500">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
                                    <div class="flex flex-col gap-3">
                                        <div class="flex justify-between items-center">
                                            <label class="text-[10px] font-bold text-secondary uppercase tracking-widest font-mono">Selling Price (PHP)</label>
                                        ...
                                        <InputNumber v-model="product.selling_price" mode="currency" currency="PHP" locale="en-PH" 
                                                    inputClass="!bg-panel/50 !border-panel-border !text-primary !h-12 !w-full !px-4 !font-mono font-bold"
                                                    :class="{'!border-red-500/50': errors.selling_price}" />
                                    </div>
                                    <div class="flex flex-col gap-3">
                                        <div class="flex justify-between items-center">
                                            <label class="text-[10px] font-bold text-secondary uppercase tracking-widest font-mono">Minimum Stock Level</label>
                                    ...
                                    <div class="flex flex-col gap-3">
                                        <div class="flex justify-between items-center">
                                            <label class="text-[10px] font-bold text-secondary uppercase tracking-widest font-mono">Valuation Method</label>
                                        ...
                                        <p v-if="isEditing && product.has_history" class="text-[8px] font-bold text-muted uppercase tracking-widest font-mono mt-1 italic">Costing method locked due to audit history</p>
                                    </div>
                                    <div class="col-span-12 md:col-span-2 p-8 bg-panel/30 border border-panel-border/60 rounded-xl relative overflow-hidden group">
                                        <div class="flex justify-between items-center relative z-10">
                                            <div class="flex flex-col gap-1">
                                                <h4 class="text-primary font-bold text-sm m-0 uppercase tracking-tight">Active Availability</h4>
                                                <p class="text-secondary text-[11px] font-mono tracking-tighter uppercase leading-none mt-1">Status: {{ product.is_active ? 'Active' : 'Inactive' }}</p>
                                            </div>
                            ...
                            <div v-show="activeTab === 'media'" class="animate-in slide-in-from-right-4 duration-500 h-full">
                                <div class="h-96 border-2 border-dashed border-panel-border bg-deep/50 rounded-2xl flex items-center justify-center relative group hover:border-sky-500/20 transition-all cursor-pointer overflow-hidden shadow-inner" 
                                    ...
                                    <div v-else class="flex flex-col items-center gap-4 text-center opacity-30 group-hover:opacity-60 transition-opacity">
                                        <div class="w-20 h-20 rounded-full bg-panel flex items-center justify-center border border-panel-border">
                                            <i class="pi pi-cloud-upload text-3xl"></i>
                                        </div>
                                        <div class="flex flex-col gap-1">
                                            <h5 class="text-primary font-bold uppercase text-xs tracking-[0.2em] m-0">Upload Product Image</h5>
                                            <span class="text-[10px] text-secondary font-mono">WEBP // PNG // JPEG (MAX: 5MB)</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div v-show="activeTab === 'packaging'" class="animate-in slide-in-from-right-4 duration-500 h-full flex flex-col">
                                <div v-if="product.uom_id" class="flex flex-col gap-6">
                                    <div class="flex justify-between items-center bg-panel/30 p-4 border border-panel-border/80 rounded-xl">
                                        <div class="flex flex-col">
                                            <span class="text-primary font-bold text-sm tracking-tight uppercase">Product Packaging Equivalency</span>
                                            <span class="text-[10px] text-secondary font-mono mt-1 leading-relaxed">Define how cases, pallets, or boxes break down accurately to the atomic base unit for true scale calculations.</span>
                                        </div>
                                        <Button v-if="!addingRule" 
                                                label="ADD PACKAGING" icon="pi pi-plus" 
                                                :disabled="!product.id"
                                                class="!bg-fuchsia-500 hover:!bg-fuchsia-400 !border-none !text-[10px] !font-bold !h-10 !px-4 disabled:!opacity-30 disabled:!bg-panel-hover disabled:!text-secondary transition-all font-mono" 
                                    ...
                                    <div v-if="!product.id && isNonBaseUomSelected" class="bg-[radial-gradient(ellipse_at_top,rgba(56,189,248,0.05),transparent)] border border-sky-500/20 p-8 rounded-xl flex flex-col gap-6 shadow-inner animate-in fade-in zoom-in duration-500">
                                        <div class="flex items-start gap-4 pb-4 border-b border-sky-500/10">
                                            ...
                                            <div class="flex flex-col gap-1">
                                                ...
                                                <p class="text-[10px] text-secondary font-mono leading-relaxed m-0 italic">No global rule found for <span class="text-primary">{{ uoms.find(u => u.id === product.uom_id)?.abbreviation }}</span>. Define it now to save this product.</p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 py-2">
                                            <div class="flex flex-col gap-3">
                                                <div class="flex justify-between items-center px-1">
                                                    <label class="text-[9px] font-black text-secondary uppercase tracking-widest font-mono italic">Configuration Rule</label>
                                                ...
                                                <div class="relative">
                                                     <InputNumber v-model="product.initial_conversion_factor" placeholder="E.g. 12" 
                                                                 class="!w-full"
                                                                 inputClass="!bg-panel/80 !border-sky-500/30 !text-primary !h-12 !px-4 !font-black !text-lg !font-mono focus:!border-sky-500 shadow-[0_0_15px_rgba(56,189,248,0.1)]" />
                                                     <div class="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-bold text-secondary tracking-tighter uppercase font-mono">{{ getBaseUomForSelected?.abbreviation ?? '???' }}</div>
                                                </div>
                                            </div>

                                            <div class="bg-panel/40 border border-panel-border p-5 rounded-lg flex items-center gap-4">
                                                <i class="pi pi-lock-open text-muted text-lg"></i>
                                                <p class="text-[9px] text-secondary font-mono leading-relaxed m-0">This rule will be saved <span class="text-sky-400 font-bold italic">Atopically</span> with the product. No global settings will be affected.</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div v-if="addingRule" class="bg-[radial-gradient(ellipse_at_top,rgba(217,70,239,0.05),transparent)] border border-fuchsia-500/20 p-6 rounded-xl flex flex-col gap-5 shadow-inner">
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <div class="flex flex-col gap-2">
                                                ...
                                                <Select v-model="newRule.from_uom_id" :options="uoms.filter(u => !u.is_base)" optionLabel="name" optionValue="id" placeholder="E.g. Box" class="!bg-panel !border-panel-border !text-primary" />
                                            </div>
                                            <div class="flex flex-col gap-2">
                                                <label class="text-[10px] font-bold text-fuchsia-400/80 uppercase tracking-widest font-mono">2. Translates To</label>
                                                <Select v-model="newRule.to_uom_id" :options="uoms.filter(u => u.is_base)" optionLabel="name" optionValue="id" placeholder="E.g. Pieces" class="!bg-panel !border-panel-border !text-sky-400 !font-bold" />
                                            </div>
                                            <div class="flex flex-col gap-2">
                                                <label class="text-[10px] font-bold text-fuchsia-400/80 uppercase tracking-widest font-mono">3. Qty in Base Unit</label>
                                                <InputNumber v-model="newRule.conversion_factor" placeholder="E.g. 12" inputClass="!bg-panel !border-panel-border !text-fuchsia-400 !font-bold !font-mono" />
                                            </div>
                                        </div>
                                    ...
                                    <div class="flex flex-col gap-3" v-if="!loadingConversions">
                                        <div v-for="rule in productConversions" :key="rule.id" class="flex justify-between items-center p-4 bg-panel/50 border border-panel-border rounded-lg group hover:border-panel-border-hover transition-colors">
                                            ...
                                            <Button icon="pi pi-trash" class="!w-8 !h-8 !p-0 !bg-transparent !border-none !text-muted hover:!text-red-400 opacity-0 group-hover:opacity-100 transition-all cursor-pointer" @click="deleteProductConversion(rule.id)" />
                                        </div>
                                        <div v-if="productConversions.length === 0 && !addingRule" class="text-center py-12 px-6 opacity-40 grayscale flex flex-col items-center">
                                            <i class="pi pi-box text-3xl mb-4 text-secondary font-light"></i>
                                            <p class="text-[10px] font-mono uppercase tracking-[0.2em] leading-relaxed m-0 text-secondary">No custom packaging defined.<br/><span class="opacity-70 text-[9px]">Product will use standard universal metrics from global settings.</span></p>
                                        </div>
                                    </div>
                ...
                <div v-if="selectedProductForStock" class="flex flex-col w-72">
                    <div class="px-4 py-3 border-b border-panel-border bg-panel/30 flex justify-between items-center">
                        ...
                        <div class="flex flex-col items-end">
                            <span class="text-xs font-black text-primary font-mono">{{ getScaledQty(selectedProductForStock, selectedProductForStock.total_qoh) }}</span>
                            <span class="text-[8px] font-bold text-muted uppercase tracking-tighter">Total On Hand</span>
                        </div>
                    </div>
