    ...
    <AppLayout>
        ...
        <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8 print:py-0 print:px-0">
            ...
            <div v-if="loading" class="py-32 flex flex-col items-center justify-center space-y-4">
                ...
                <p class="font-mono text-[10px] uppercase tracking-[0.3em] text-zinc-600 animate-pulse">Retrieving Settlement Data...</p>
            </div>
                ...
                <div class="bg-white text-zinc-900 rounded-2xl shadow-[0_32px_64px_-16px_rgba(0,0,0,0.5)] border-t-8 border-amber-500 overflow-hidden relative z-10 print:shadow-none print:border-none print:rounded-none">
                    <div class="p-12">
                        ...
                        <div class="grid grid-cols-2 gap-12 mb-8">
                            <!-- Paid To -->
                            <div>
                                <div class="text-xs font-bold uppercase tracking-widest text-zinc-400 mb-2">Paid To</div>
                                <div class="font-bold text-lg text-zinc-900">{{ payment.vendor?.name }}</div>
                                <div class="text-sm text-zinc-600 mt-1 uppercase font-mono">{{ payment.vendor?.vendor_code }}</div>
                            ...
                            <table class="w-full text-left border-collapse">
                                ...
                                <tbody class="divide-y divide-zinc-100 font-mono">
                                    <tr v-if="!payment.allocations?.length && !payment.refunds?.length">
                                        <td colspan="3" class="px-2 py-8 text-center text-zinc-400 text-xs italic tracking-widest">No primary allocations recorded</td>
                                    </tr>
                                    <!-- Regular Allocations -->
                        ...
                        <div class="flex justify-end">
                            <div class="w-72">
                                <div class="flex justify-between py-2 border-b border-zinc-100">
                                    <span class="text-xs font-bold uppercase tracking-widest text-zinc-400">Total Funds Sent</span>
        ...
        <Dialog v-model:visible="showAllocateDialog" modal :style="{ width: '50rem' }" class="p-fluid">
            <template #header>
                <div class="flex flex-col">
                    ...
                    <span class="text-xs text-zinc-400">Apply leftover funds to open bills for {{ payment?.vendor?.name }}</span>
                </div>
            </template>
            ...
            <div class="bg-zinc-900 border border-zinc-800 rounded-xl p-6 shadow-xl flex items-center justify-between mb-6 mt-4">
                ...
                <button @click="autoAllocate" v-if="allocateItems.length > 0" class="px-4 py-2 bg-sky-500/10 border border-sky-500/20 text-sky-400 hover:bg-sky-500 hover:text-white rounded-lg text-xs font-bold uppercase tracking-widest transition-colors">
                    Auto Allocate
                </button>
            </div>

            <div v-if="loadingBills" class="flex justify-center p-8 text-zinc-500">
                <i class="pi pi-spinner pi-spin text-3xl"></i>
            </div>
            <div v-else-if="allocateItems.length === 0" class="p-8 text-center text-zinc-500 border border-dashed border-zinc-800 rounded-xl">
                    ...
                    <Column header="Bill">
                        <template #body="{ data }">
                            <div class="flex flex-col py-1">
                                <span class="text-sm text-zinc-200 font-bold bg-zinc-950 px-2 py-1 rounded border border-zinc-800 self-start mb-1">{{ data.bill_number }}</span>
                                <span class="text-[10px] text-zinc-500 ml-1">{{ data.bill_date }}</span>
                            </div>
                        </template>
                    </Column>
                    ...
                    <Column header="Apply Amount" style="width: 35%" class="text-right">
                        <template #body="{ data }">
                            <InputNumber v-model="data.amount" mode="currency" currency="PHP" locale="en-PH"
                                         class="w-full" inputClass="bg-zinc-950 border-zinc-700 text-amber-500 font-bold text-right text-sm w-full focus:border-amber-500 outline-none" />
                        </template>
                    </Column>
                </DataTable>
            </div>
        ...
        <Dialog v-model:visible="showRefundDialog" modal :style="{ width: '32rem' }" class="p-fluid">
            <template #header>
                <div class="flex flex-col">
                    ...
                    <span class="text-xs text-zinc-400">Return unallocated credit from {{ payment?.vendor?.name }}</span>
                </div>
            </template>
            ...
            <div class="space-y-4">
                <div class="flex flex-col gap-2">
                    ...
                    <InputNumber v-model="refundForm.amount" :min="0.01" :max="Number(payment?.unallocated_amount)" :minFractionDigits="2" :maxFractionDigits="2"
                                 inputClass="!bg-zinc-900 !border-zinc-700 !text-rose-400 !font-bold !text-lg outline-none" />
                </div>
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <Select v-model="refundForm.refund_method" :options="refundMethods" 
                                class="!bg-zinc-900 !border-zinc-700 !text-zinc-300 outline-none" />
                    </div>
                </div>
                <div class="flex flex-col gap-2">
                    ...
                    <InputText v-model="refundForm.reference_number" placeholder="e.g. CHK-001234"
                               class="!bg-zinc-900 !border-zinc-700 !text-zinc-300 !h-11 outline-none" />
                </div>
                <div class="flex flex-col gap-2">
