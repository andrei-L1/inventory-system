        ...
        <div class="p-8 max-w-7xl mx-auto font-sans">
            ...
            <div class="mb-10 flex items-center justify-between">
                <div class="flex items-center gap-6">
                    <button @click="router.visit('/finance-center?mode=PAYABLE')" 
                            class="w-12 h-12 rounded-2xl bg-panel border border-panel-border flex items-center justify-center text-secondary hover:text-primary hover:border-zinc-700 transition-all shadow-lg">
                        <i class="pi pi-arrow-left"></i>
                    </button>
                    <div>
                        <div class="flex items-center gap-3 mb-1">
                            <h1 class="text-3xl font-black text-primary tracking-tight uppercase">
            ...
            <div class="grid grid-cols-12 gap-10">
                ...
                <div class="col-span-12 lg:col-span-4 space-y-8">
                    <div class="bg-panel/40 border border-panel-border rounded-3xl p-8 space-y-8 backdrop-blur-xl shadow-2xl relative overflow-hidden group">
                        ...
                        <div class="space-y-3">
                            <label class="text-[11px] font-black text-secondary uppercase tracking-[0.2em] px-1 flex items-center gap-2">
                            ...
                            <Select v-model="selectedPo" :options="purchaseOrders" optionLabel="po_number" placeholder="Optional: Select PO to Bill" 
                                    class="w-full bg-deep/80 border-panel-border text-primary rounded-xl h-12" 
                                    :loading="loadingPOs" filter @change="onPoSelect" showClear />
                            <div v-if="!isPOLinked" class="px-1 text-[9px] text-muted font-bold uppercase tracking-tight">
                        ...
                        <div class="space-y-3">
                            ...
                            <Select v-model="selectedVendor" :options="vendors" optionLabel="name" placeholder="Select Vendor" 
                                    :disabled="isPOLinked"
                                    class="w-full bg-deep/80 border-panel-border text-primary rounded-xl h-12" filter />
                            <div v-if="isPOLinked" class="flex items-center gap-2 px-1">
                                <i class="pi pi-lock text-[9px] text-muted"></i>
                                <span class="text-[9px] text-muted font-bold uppercase tracking-widest italic">Locked to Order Source</span>
                            </div>
                        </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <input v-model="form.bill_number" type="text" 
                                   class="w-full bg-deep/80 border border-panel-border rounded-xl h-12 px-5 text-primary text-sm font-mono focus:border-amber-500/50 focus:ring-4 focus:ring-amber-500/10 outline-none transition-all placeholder:text-muted"
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-secondary uppercase tracking-[0.2em] px-1">Issue Date</label>
                                <DatePicker v-model="form.bill_date" class="w-full" dateFormat="yy-mm-dd" 
                                            inputClass="!bg-deep/80 !border-panel-border !text-primary !font-mono !text-xs !rounded-xl !h-12 !px-4" />
                            </div>
                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-secondary uppercase tracking-[0.2em] px-1">Due Date</label>
                        ...
                        <!-- Financial Summary -->
                        <div class="pt-4 border-t border-panel-border/50">
                            <div class="bg-deep p-6 rounded-2xl border border-panel-border shadow-inner">
                                <div class="flex justify-between items-center mb-1">
                                    <span class="text-[10px] font-black text-secondary uppercase tracking-widest">Grand Liability Total</span>
                                    <div class="flex items-center gap-2">
                                        <div class="w-2 h-2 rounded-full bg-amber-500 animate-pulse"></div>
                                        <span class="text-[10px] font-bold text-amber-500 uppercase">PHP Balance</span>
                                    </div>
                                </div>
                                <div class="text-4xl font-black text-primary font-mono tracking-tighter">
                    ...
                    <!-- Audit Trail Sidebar -->
                    <div v-if="isPOLinked" class="bg-panel/20 border border-panel-border/50 rounded-3xl p-8 space-y-6">
                        <div class="flex items-center justify-between">
                            <span class="text-[11px] font-black text-muted uppercase tracking-[0.2em]">Transaction History</span>
                            <span class="text-[9px] font-bold text-muted uppercase tracking-widest">Audit Trail</span>
                        </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div v-for="rcpt in linkedPO?.receipts" :key="rcpt.id" class="flex items-start gap-4 group">
                                ...
                                <div>
                                    ...
                                    <div class="text-[9px] text-muted font-bold uppercase tracking-tighter">Received on {{ rcpt.received_at }}</div>
                                </div>
                            </div>
                            <!-- Returns -->
                            <div v-for="rtn in linkedPO?.returns" :key="rtn.id" class="flex items-start gap-4 group">
                                ...
                                <div>
                                    ...
                                    <div class="text-[9px] text-muted font-bold uppercase tracking-tighter">Stock RTV on {{ rtn.returned_at }}</div>
                                </div>
                            </div>
                        </div>
                        <p class="text-[9px] text-muted italic font-medium pt-2 border-t border-panel-border/30">
                ...
                <div class="col-span-12 lg:col-span-8">
                    <div class="bg-panel border border-panel-border rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col min-h-[700px]">
                        <!-- Table Header -->
                        <div class="p-8 border-b border-panel-border flex justify-between items-center bg-panel/50 backdrop-blur-md sticky top-0 z-20">
                            <div>
                                <h3 class="text-primary font-black text-lg tracking-tight flex items-center gap-3">
                                ...
                                <p class="text-[10px] text-secondary font-bold uppercase tracking-widest mt-1 ml-11 italic">Matching Warehouse Artifacts to Ledger Entries</p>
                            </div>
                            
                            <div v-if="isPOLinked" class="px-4 py-2 bg-deep border border-panel-border rounded-xl flex items-center gap-3">
                                <div class="flex items-center gap-2">
                                    <div class="w-1.5 h-1.5 rounded-full bg-blue-500"></div>
                                    <span class="text-[9px] font-black text-secondary uppercase tracking-widest">Receipt</span>
                                </div>
                                <div class="w-px h-3 bg-panel-hover"></div>
                                <div class="flex items-center gap-2">
                                    <div class="w-1.5 h-1.5 rounded-full bg-rose-500"></div>
                                    <span class="text-[9px] font-black text-secondary uppercase tracking-widest">Return</span>
                                </div>
                            </div>
                        </div>

                        <!-- Grouped Content -->
                        <div class="flex-1 overflow-y-auto p-8 space-y-12 bg-deep/20">
                            <div v-for="group in groupedLines" :key="group.po_line_id" class="space-y-4">
                                <!-- PO Line Header -->
                                <div class="flex flex-col gap-2 relative">
                                    <div class="flex items-end justify-between px-2">
                                        <div class="flex flex-col">
                                            <span class="text-[9px] font-black text-muted uppercase tracking-[0.3em] mb-1">Product Origin</span>
                                            <h4 class="text-primary font-black text-base tracking-tight leading-none">{{ group.product_name }}</h4>
                                            <span class="text-[10px] font-mono text-secondary font-bold mt-1 uppercase opacity-60 tracking-tighter">{{ group.sku }}</span>
                                        </div>
                                        <div class="flex items-center gap-6 pb-1">
                                            <div class="flex flex-col items-end">
                                                <span class="text-[9px] font-black text-muted uppercase tracking-widest">Ordered</span>
                                                <span class="text-xs font-bold text-secondary font-mono">{{ group.ordered_qty }} {{ group.uom }}</span>
                                            </div>
                                            <div class="w-px h-6 bg-panel-hover/50"></div>
                                            <div class="flex flex-col items-end">
                                                <span class="text-[9px] font-black text-muted uppercase tracking-widest">Net Billable</span>
                                                <div class="flex items-center gap-1.5">
                                                    <span class="text-xs font-black text-amber-500 font-mono">{{ group.billable_qty.toLocaleString() }}</span>
                                                    <span class="text-[9px] font-bold text-muted uppercase">{{ group.uom }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <!-- Transaction Table -->
                                <div class="bg-panel/30 rounded-2xl border border-panel-border/50 overflow-hidden">
                                    <table class="w-full border-collapse">
                                        <thead class="bg-deep/40">
                                            <tr class="text-[9px] font-black text-muted uppercase tracking-widest border-b border-panel-border">
                                            ...
                                            <tr v-for="r in group.receipts" :key="r.transaction_line_id" class="border-b border-zinc-900/50 group/row hover:bg-blue-500/[0.02] transition-colors">
                                                ...
                                                <td class="py-4 px-4">
                                                    <div class="flex flex-col">
                                                        ...
                                                        <span class="text-[8px] text-muted font-bold uppercase tracking-tighter">{{ r.date }}</span>
                                                    </div>
                                                </td>
                                                <td class="py-4 px-4 text-right">
                                                    <div class="flex flex-col items-end">
                                                        <div class="flex items-baseline gap-1">
                                                            <span class="text-xs font-bold text-primary font-mono">{{ r.quantity_in_po_unit.toLocaleString() }} {{ r.uom }}</span>
                                                            <span class="text-[9px] text-muted font-bold font-mono">[{{ r.pieces.toLocaleString() }} {{ r.base_uom }}]</span>
                                                        </div>
                                                        <span v-if="r.billed_in_po_unit > 0" class="text-[8px] text-muted font-bold italic">({{ r.billed_in_po_unit }} {{ r.uom }} already billed)</span>
                                                    </div>
                                                </td>
                                                <td class="py-4 px-4 text-right">
                                                     <span class="text-[11px] font-mono text-primary">{{ formatCurrency(r.unit_price) }}</span>
                                                </td>
                                                <td class="py-4 px-2 text-center">
                                                    <InputNumber v-model="r.discount_rate" :min="0" :max="100" fluid inputClass="!bg-deep/50 !border-panel-border !text-rose-400 !font-black !text-xs !text-center !py-2 !rounded-lg" />
                                                </td>
                                                <td class="py-4 px-2 text-center">
                                                    <InputNumber v-model="r.tax_rate" :min="0" :max="100" fluid inputClass="!bg-deep/50 !border-panel-border !text-blue-400 !font-black !text-xs !text-center !py-2 !rounded-lg" />
                                                </td>
                                                <td class="py-4 px-6 text-right">
                                                    <div class="relative group/input">
                                                        <InputNumber v-model="r.bill_qty" :min="0" :max="r.available_to_bill" :step="0.01" fluid
                                                                    inputClass="!bg-deep !border-amber-500/20 focus:!border-amber-500 !text-primary !font-black !text-xs !text-right !py-2 !rounded-lg !shadow-inner transition-all" />
                                                        <div class="flex flex-col items-end mt-1 px-1">
                                                            <span class="text-primary font-mono text-xs font-black">{{ formatCurrency(subtotal(r)) }}</span>
                                                            <span class="text-[8px] text-muted font-bold font-mono uppercase tracking-tighter">Matches [{{ (r.bill_qty * r.factor).toLocaleString() }} {{ r.base_uom }}]</span>
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
                                            ...
                                            <tr v-for="rtn in group.returns" :key="rtn.reference" class="border-b border-zinc-900/50 bg-rose-500/[0.01] hover:bg-rose-500/[0.03] transition-colors">
                                                <td class="py-4 px-6">
                                                    <div class="flex items-center gap-3 opacity-60">
                                                        ...
                                                        <span class="text-[11px] font-black font-mono text-secondary">{{ rtn.reference }}</span>
                                                    </div>
                                                </td>
                                                <td class="py-4 px-4">
                                                    <div class="flex flex-col opacity-60">
                                                        ...
                                                        <span class="text-[8px] text-muted font-bold uppercase tracking-tighter">{{ rtn.date }}</span>
                                                    </div>
                                                </td>
                                                ...
                                                <td class="py-4 px-4 opacity-30 text-right text-[10px] font-bold text-muted uppercase italic">Non-Billable</td>
                                                <td class="py-4 px-2"></td>
                                                <td class="py-4 px-2"></td>
                                                <td class="py-4 px-6 text-right opacity-30 pointer-events-none grayscale">
                                                    <div class="h-8 w-full bg-deep/50 rounded-lg flex items-center justify-center">
                                                        <i class="pi pi-ban text-[8px] text-muted"></i>
                                                    </div>
                                                </td>
                                            </tr>
                                        </tbody>
                            ...
                            <div v-if="!isPOLinked" class="space-y-6">
                                <div v-for="(line, idx) in form.lines" :key="idx" class="bg-panel/40 p-6 rounded-3xl border border-panel-border grid grid-cols-12 gap-6 items-center group">
                                    <div class="col-span-12 lg:col-span-4 space-y-2">
                                        <label class="text-[9px] font-black text-muted uppercase tracking-widest px-1">Description</label>
                                        <input v-model="line.description" type="text" 
                                               class="w-full bg-deep border border-panel-border rounded-xl h-11 px-4 text-primary text-xs focus:border-amber-500 outline-none transition-all">
                                    </div>
                                    <div class="col-span-4 lg:col-span-2 space-y-2">
                                        <label class="text-[9px] font-black text-muted uppercase tracking-widest px-1">Qty</label>
                                        <InputNumber v-model="line.quantity" :min="0" fluid 
                                                     inputClass="!bg-deep !border-panel-border !text-primary !font-mono !text-xs !text-right !h-11 !rounded-xl" />
                                    </div>
                                    <div class="col-span-4 lg:col-span-2 space-y-2">
                                        <label class="text-[9px] font-black text-muted uppercase tracking-widest px-1">Price</label>
                                    ...
                                    <div class="col-span-2 lg:col-span-1 space-y-2">
                                        <label class="text-[9px] font-black text-muted uppercase tracking-widest px-1">Disc %</label>
                                        <InputNumber v-model="line.discount_rate" :min="0" :max="100" fluid inputClass="!bg-deep text-rose-400 !border-panel-border !text-center !text-xs !h-11 !rounded-xl" />
                                    </div>
                                    <div class="col-span-2 lg:col-span-1 space-y-2">
                                        <label class="text-[9px] font-black text-muted uppercase tracking-widest px-1">Tax %</label>
                                        <InputNumber v-model="line.tax_rate" :min="0" :max="100" fluid inputClass="!bg-deep text-blue-400 !border-panel-border !text-center !text-xs !h-11 !rounded-xl" />
                                    </div>
                                    <div class="col-span-3 lg:col-span-1 text-right pt-5">
                                        <div class="text-[10px] font-black text-primary font-mono opacity-50">{{ formatCurrency(subtotal(line)) }}</div>
                                    </div>
                                    <div class="col-span-1 pt-5">
                                        <button @click="form.lines.splice(idx, 1)" v-if="form.lines.length > 1"
                                                class="w-9 h-9 rounded-xl hover:bg-rose-500/10 text-muted hover:text-rose-500 transition-all flex items-center justify-center">
                                            <i class="pi pi-trash text-xs"></i>
                                        </button>
                                    </div>
                                </div>
                                <button @click="form.lines.push({ description: '', quantity: 1, unit_price: 0, discount_rate: 0, tax_rate: 0 })" 
                                        class="w-full h-12 border-2 border-dashed border-panel-border rounded-2xl text-muted hover:text-amber-500 hover:border-amber-500/50 hover:bg-amber-500/5 transition-all text-[10px] font-black uppercase tracking-[0.2em] flex items-center justify-center gap-3">
...
:deep(.p-datepicker) {
    @apply bg-deep border border-panel-border rounded-2xl shadow-2xl;
}
...
::-webkit-scrollbar-thumb {
    @apply bg-panel-hover rounded-full hover:bg-zinc-700 transition-colors;
}
</style>
