...
const colorMap = {
    ...
    '#58a6ff': 'var(--text-primary)', // GitHub blue to primary text or keep it accent? The design is monochrome slate. pure white accent.
    '#1f6feb': 'var(--text-muted)',
    '#238636': 'var(--accent-primary)', // Used to be green button, now pure white
    'rgba\\(35, 134, 54, 0.7\\)': 'rgba(250, 250, 250, 0.7)',
    '#57ab5a': 'var(--accent-primary)', // success green to white
...
function processDirectory(dir) {
    ...
    for (const file of files) {
        ...
        if (fs.statSync(fullPath).isDirectory()) {
            ...
            // Fix specific gradient strings first to avoid partial matches
            ...
            for (const [hex, variable] of Object.entries(colorMap)) {
                if (hex.includes('gradient')) continue; // Skip gradients in this loop
